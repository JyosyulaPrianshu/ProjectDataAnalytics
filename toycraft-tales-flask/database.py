...
class DatabaseManager:
    ...
    def has_user_reward(self, user_email, reward_type, reward_value, description):
        """Return True if the user has already received a specific reward."""
        connection = None
        cursor = None
        try:
            connection = self.get_connection()
            ...
            cursor = connection.cursor()
            cursor.execute(
                "SELECT id FROM user_rewards WHERE user_email = %s AND reward_type = %s AND reward_value = %s AND description = %s LIMIT 1",
                (user_email, reward_type, reward_value, description)
            )
            return cursor.fetchone() is not None
        except Exception as e:
            print(f"❌ Error checking user reward: {e}")
            return False
        finally:
            if cursor:
                cursor.close()
